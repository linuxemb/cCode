...
void arrayqueue_init(arrayqueue_t* q, size_t elem_size, size_t initial_capacity) {
    assert(q != NULL);
    assert(elem_size > 0);

    q->array = malloc(elem_size * initial_capacity);  // Allocate space for multiple elements
    assert(q->array != NULL);
